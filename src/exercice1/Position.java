...
public class Position implements Cloneable {
	...
	/**
	 * The method in position responsible for associating the train with the next
	 * possible position
	 * 
	 * @return the next position for the train
	 */

	public Position nextPosition() {
		Element elet = pos.getNext(direction);
		if (elet == pos) {
			return new Position(elet, direction.change());
		} else {
			elet.enter();
			pos.leave();
			return new Position(elet, direction);
		}
